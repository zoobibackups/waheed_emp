...
export default function Example(props) {
    ...
    let handleMessage = useCallback(({type, id, data}) => {
        ...
        switch (type) {
            case 'ImgClick':
                richText.current?.commandDOM(`$('#${id}').src="${imageList[Math.random(imageList.length - 1)]}"`);
                break;
            case 'TitleClick':
                const color = ['red', 'blue', 'gray', 'yellow', 'coral'];

                // command: $ = document.querySelector
                richText.current?.commandDOM(`$('#${id}').style.color='${color[Math.random(color.length - 1)]}'`);
                break;
            case 'SwitchImage':
                break;
        }
        ...
        <SafeAreaView style={[styles.container, dark && styles.darkBack]}>
            ...
            <ScrollView
                ...
                <View style={[styles.topVi, dark && styles.darkBack]}>
                    ...
                    <View style={styles.item}>
                        <Button title={theme === "light" ? "dark":"light"} onPress={onTheme}/>
