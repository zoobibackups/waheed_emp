...
const AddTimeSheetScreen = ({navigation}) => {
  ...
  const getJobtimetype = itemValue => {
    ...
    getJobWorkingDays(user.account_id, itemValue)
      .then(response => {
        if (response.status === 200) {
          //     console.log(response.data.data[0].working_days_config);
  ...
  function getISOWeekDates(isoWeekNum = 1, year = new Date().getFullYear()) {
    ...
    for (var dates = [], i = 0; i < 7; i++) {
      dates.push({
        date: d.format('ddd DD MMM'),
        insert_date: d.format('YYYY-MM-DD'),
        hours: null,
      });
      d.add(1, 'day');
    }
  ...
  const getNumberofdays = async date => {
    ...
    if (time_sheet_type === 'Week') {
      ...
      setWeekDays([
        {
          date: weekdays,
          insert_date: weekdays.format('YYYY-MM-DD'),
          hours: null,
        },
      ]);
      setAlldata([[...weekdays]]);
  ...
  const changeTab = async value => {
    ...
    if (value === 'Week') {
      ...
      setWeekDays([
        {
          date: wdays,
          insert_date: moment(startDate).format('YYYY-MM-DD'),
          hours: null,
        },
      ]);
    }
